	...
	BView(r, "BeNESPanelView", B_FOLLOW_ALL, B_WILL_DRAW) 
{
	...
	BRadioButton* radio = new BRadioButton(
		BRect(10, 25, 100-1, 50-1),
		"keyboard_radio",
		"Keyboard",
	...
	radio = new BRadioButton(
		BRect(10, 50, 100-1, 75-1),
		"joystick_radio",
		"Joystick",
	...
	BCheckBox* check = new BCheckBox(
		BRect(140, 58, 235-1, 78-1),
		CHECKBOX_FULLSCREEN,
		"Full screen",
	...
	BWindow(frame, "BeNES: Control Panel", B_TITLED_WINDOW, B_NOT_RESIZABLE)
{
	...
	openPanel->SetPanelDirectory(
		BNPreference::Instance()->getROMdirectory());
	Unlock();
}

bool BNPanel::QuitRequested()
{
	be_app->PostMessage(B_QUIT_REQUESTED, be_app);
	return true;
}

void BNPanel::MessageReceived(BMessage* msg)
{
	Lock();
	BNPreference* pref = BNPreference::Instance();
	BNJoypad* joy = BNJoypad::Instance();
