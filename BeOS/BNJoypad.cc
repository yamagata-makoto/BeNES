...
/*--------------------------------------------- BNKeyboardGrip ---*/
inline bool
isKeyDown(key_info& info, unsigned char key)
{
...
void
BNKeyboardGrip::get(JoypadProtocol* joy)
{
  static key_info info;

  get_key_info(&info);
  if (isKeyDown(info, UP))
    joy->push(NESK_UP);
  else
    joy->release(NESK_UP);

  if (isKeyDown(info, DOWN))
    joy->push(NESK_DOWN);
  else
    joy->release(NESK_DOWN);

  if (isKeyDown(info, LEFT))
    joy->push(NESK_LEFT);
  else
    joy->release(NESK_LEFT);

  if (isKeyDown(info, RIGHT))
    joy->push(NESK_RIGHT);
  else
    joy->release(NESK_RIGHT);

  if (isKeyDown(info, Z) || isKeyDown(info, LALT)) {
    joy->push(NESK_A);
  } else
    joy->release(NESK_A);

  if (isKeyDown(info, X) || isKeyDown(info, LSHIFT) || isKeyDown(info, LCTRL)) {
    joy->push(NESK_B);
  } else
    joy->release(NESK_B);

  if (isKeyDown(info, RETURN))
    joy->push(NESK_START);
  else
    joy->release(NESK_START);

  if (isKeyDown(info, SPACE))
    joy->push(NESK_SELECT);
  else
    joy->release(NESK_SELECT);
}
...
void
BNJoystickGrip::get(JoypadProtocol* joy)
{
  ...
  if (*(axesstat + 1) < NTOLERANCE)
    joy->push(NESK_UP);
  else
    joy->release(NESK_UP);

  if (*(axesstat) > PTOLERANCE)
    joy->push(NESK_RIGHT);
  else
    joy->release(NESK_RIGHT);

  if (*(axesstat) < NTOLERANCE)
    joy->push(NESK_LEFT);
  else
    joy->release(NESK_LEFT);
}
...
void
BNJoypad::startPolling()
{
  if (id == 0) {
    running = true;
    id = spawn_thread(BNJoypad::Polling, "BNJoypad", B_NORMAL_PRIORITY, this);
    resume_thread(id);
  }
}
...
int32
BNJoypad::Polling(void* arg)
{
  JoypadProtocol* joyA = JoypadA();
  BNJoypad* obj = reinterpret_cast<BNJoypad*>(arg);
...
void
BNJoypad::setInputDeviceType(int type)
{
  switch (type) {
