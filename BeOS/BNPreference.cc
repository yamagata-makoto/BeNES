...
inline void readString(FILE* fp, char* buff, size_t size)
{
	fgets(buff, size, fp);
}

inline void writeString(FILE* fp, char* buff)
{
	fputs(buff, fp);
}

inline int readInteger(FILE* fp)
{
	char buff[BUFSIZ];
...
inline void writeInteger(FILE* fp, int val)
{
	char buff[BUFSIZ];
	sprintf(buff, "%d\n", val);
	writeString(fp, buff);
