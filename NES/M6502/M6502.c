...
#define M_ADC(Rg) \
  ...
  { \
    ...
    K.B.h=(R->A>>4)+(Rg>>4)/*+(K.B.l>15? 1:0) remove by makoto*/; \
...
#define M_SBC(Rg) \
  ...
  { \
    ...
    /*K.B.h=(R->A>>4)-(Rg>>4)-(K.B.l&0x10); changed by makoto*/ \
...
void Int6502(M6502 *R,byte Type)
{
  ...
  if((Type==INT_NMI)||((Type==INT_IRQ)&&!(R->P&I_FLAG)))
  {
    ...
    M_PUSH(R->P&~B_FLAG);
    R->P&=~D_FLAG;
    if(Type==INT_NMI) { R->P|=I_FLAG;J.W=0xFFFA; } else { R->P|=I_FLAG;J.W=0xFFFE; }
    R->PC.B.l=Rd6502(J.W++);
    R->PC.B.h=Rd6502(J.W);
  }
}
...
word Run6502(M6502 *R)
{
    ...
    J.W = 0; /* gcc -O2 is a chode */
    K.W = 0; /* gcc -O2 is a chode */
    
    R->ICount+=R->IPeriod;     /* Reset the cycle counter   */
    
    while ((R->ICount > 0) || (R->AfterCLI)) {
	K.W = R->IRequest;
	R->IRequest = INT_NONE;
	...
	if(K.W==INT_QUIT) return(R->PC.W); /* Exit if INT_QUIT     */
	if(K.W!=INT_NONE) Int6502(R,K.W);  /* Interrupt if needed  */
