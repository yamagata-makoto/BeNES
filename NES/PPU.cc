...
static void
write2000(unsigned char data);
static void
write2001(unsigned char data);
static void
write2002(unsigned char data);
static void
write2003(unsigned char data);
static void
write2004(unsigned char data);
static void
write2005(unsigned char data);
static void
write2006(unsigned char data);
static void
write2007(unsigned char data);

unsigned char
PPUread(unsigned address);
void
PPUwrite(unsigned address, unsigned char data);
...
/* private functions */
void
renderScanLine(NESscreen* screen, unsigned scanline);
void
renderScreen(char* p, char* end, unsigned scanline);
void
renderSprite(char* p, char* end, unsigned scanline);
void
getScreenTile(PatternTile* tile, unsigned address, unsigned tileY);
void
getSpriteTile(PatternTile* tile, SpriteData* data, unsigned tileY);
void
makeCachedAttrTable(unsigned address, unsigned char data);
...
void
PPUprotocol::resetVBLANK()
{
  RESET_VBLANK();
}

void
PPUprotocol::spriteDMA(unsigned char* data)
{
  memcpy(spriteRAM, data, 0x100);
}

BOOL
PPUprotocol::isExecuteNMIonVBLANK()
{
  return EXECUTE_NMI_ON_VBLANK();
}

BOOL
PPUprotocol::isScreenVisible()
{
  return IS_SCREEN_VISIBLE();
}

void
PPUprotocol::drawScanline(unsigned scanline, NESscreen* screen)
{
...
void
PPUprotocol::setDelegate(PPUdelegate* aDelegate)
{
  delegate = aDelegate;
}
...
getScreenTile(PatternTile* tile, unsigned address, unsigned tileY)
{
  ...
  unsigned ptAddress =
    SCREENPATTERN_TABLE_ADDRESS() | (NAME_TABLE(ntAddress) * 16) + tileY;
  if (delegate) {
...
void
renderSprite(char* beg, char* end, unsigned scanline)
{
  PatternTile tile;
...
write2007(unsigned char data)
{
  ...
  if (address < 0x2000) {
    ...
    if ((address == 0x3f00) || (address == 0x3f10)) {
      paletteRAM[0x00] = paletteRAM[0x04] = paletteRAM[0x08] =
        paletteRAM[0x0c] = paletteRAM[0x10] = paletteRAM[0x14] =
          paletteRAM[0x18] = paletteRAM[0x1c] =
            PaletteIndex()->toIndex(data & 0x3f);
    } else if (address & 0x03) {
      paletteRAM[address & 0x001f] = PaletteIndex()->toIndex(data & 0x3f);
    }
  }
  vramAddress += (reg2000 & 0x04) ? 32 : 1;
}
...
makeCachedAttrTable(unsigned address, unsigned char data)
{
  static const unsigned addressTable[0x40] = {
    0x000, 0x004, 0x008, 0x00c, 0x010, 0x014, 0x018, 0x01c, 0x080, 0x084, 0x088,
    0x08c, 0x090, 0x094, 0x098, 0x09c, 0x100, 0x104, 0x108, 0x10c, 0x110, 0x114,
    ...
    0x31c, 0x380, 0x384, 0x388, 0x38c, 0x390, 0x394, 0x398, 0x39c
  };
  unsigned pos = (address & 0xfc00) | addressTable[address & 0x003f];

  CACHED_ATTR_TABLE(pos + 0x00) = CACHED_ATTR_TABLE(pos + 0x01) =
    CACHED_ATTR_TABLE(pos + 0x20) = CACHED_ATTR_TABLE(pos + 0x21) =
      (data & 0x03) << 2;

  CACHED_ATTR_TABLE(pos + 0x02) = CACHED_ATTR_TABLE(pos + 0x03) =
    CACHED_ATTR_TABLE(pos + 0x22) = CACHED_ATTR_TABLE(pos + 0x23) =
      (data & 0x0c);

  CACHED_ATTR_TABLE(pos + 0x40) = CACHED_ATTR_TABLE(pos + 0x41) =
    CACHED_ATTR_TABLE(pos + 0x60) = CACHED_ATTR_TABLE(pos + 0x61) =
      (data & 0x30) >> 2;

  CACHED_ATTR_TABLE(pos + 0x42) = CACHED_ATTR_TABLE(pos + 0x43) =
    CACHED_ATTR_TABLE(pos + 0x62) = CACHED_ATTR_TABLE(pos + 0x63) =
      (data & 0xc0) >> 4;
}
