...
void Mapper001::write(unsigned address, unsigned char data)
{
	if (data & 0x80) {
...
void Mapper001::setCHRbank()
{
	// chr bank set
	if (chrrom.size > 0) {
		if (regs[0] & 0x10) {
			PatternTable()->copy4k(0x0000, 
				&(chrrom_ptr[regs[1]*0x1000]));
			PatternTable()->copy4k(0x1000, 
				&(chrrom_ptr[regs[2]*0x1000]));
		}
		else {
			PatternTable()->copy8k(0x0000,
				&(chrrom_ptr[(regs[1]&0xfe)*0x1000]));
		}
	}
}

void Mapper001::setPRGbank()
{
	int bankbase = calcBankbase();
	// prg bank set
	if (regs[0] & 0x08) {
		unsigned char* p = 
			&(prgrom.image[(regs[3]*0x4000)+bankbase]);
		if (regs[0] & 0x04) {
		...
		unsigned char* p = 
			&(prgrom_ptr[((regs[3]&0xfe)*0x4000)+bankbase]);
