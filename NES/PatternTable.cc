...
static void PTreset();
static void PTcopy1k(unsigned address, unsigned char* data);
static void PTcopy2k(unsigned address, unsigned char* data);
static void PTcopy4k(unsigned address, unsigned char* data);
static void PTcopy8k(unsigned address, unsigned char* data);
static void PTdeepCopy8k(unsigned address, unsigned char* data);

static PatternTableProtocol instance = {
	PTreset,
	PTcopy1k,
	PTcopy2k,
	PTcopy4k,
...
void PTdeepCopy8k(unsigned address, unsigned char* data)
{
	int i;
...
void PTcopy1k(unsigned address, unsigned char* data)
{
	copy(address, data, 1);
}

void PTcopy2k(unsigned address, unsigned char* data)
{
	copy(address, data, 2);
}

void PTcopy4k(unsigned address, unsigned char* data)
{
	copy(address, data, 4);
}

void PTcopy8k(unsigned address, unsigned char* data)
{
	copy(address, data, 8);
}

void copy(unsigned address, unsigned char* data, unsigned num)
{
	if (PatternTableWriteProtect == NO) return;
