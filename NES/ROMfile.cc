...
BOOL ROMfile::open(const char* file, char* err)
{
...
{
	fclose(fp);
	if (prgrom.image) free(prgrom.image);
	prgrom.image = NULL;
	prgrom.size  = 0;
	if (chrrom.image) free(chrrom.image);
	chrrom.image = NULL;
	chrrom.size  = 0;
	if (trainer.image) free(trainer.image);
	trainer.image = NULL;
	trainer.size  = 0;
...
void ROMfile::getPrgrom(ROMdata* data)
{
	deepCopyROMdata(data, &prgrom);
}

void ROMfile::getChrrom(ROMdata* data)
{
	deepCopyROMdata(data, &chrrom);
}

void ROMfile::getTrainer(ROMdata* data)
{
	deepCopyROMdata(data, &trainer);
}

unsigned ROMfile::getMapperNumber()
{
	return mapperNumber;
}

int ROMfile::getMirrorType()
{
	return mirrorType;
}

unsigned getMapperNumber(ROMfileHeader* header)
{
...
unsigned char* readROM(unsigned size, FILE* fp)
{
...
void deepCopyROMdata(ROMdata* to, ROMdata* from)
{
	to->image = NULL;
	to->size = 0;
	to->banks = 0;
