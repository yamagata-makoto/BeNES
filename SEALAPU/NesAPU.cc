...
static const unsigned char SoundLengthTbl[0x20]= {
	0x05*2-1, 0x7f*2-1, 0x0a*2-1, 0x01*2-1,
	0x14*2-1, 0x02*2-1, 0x28*2-1, 0x03*2-1,
	0x50*2-1, 0x04*2-1, 0x1e*2-1, 0x05*2-1,
	0x07*2-1, 0x06*2-1, 0x0d*2-1, 0x07*2-1,
	0x06*2-1, 0x08*2-1, 0x0c*2-1, 0x09*2-1,
	0x18*2-1, 0x0a*2-1, 0x30*2-1, 0x0b*2-1,
	0x60*2-1, 0x0c*2-1, 0x24*2-1, 0x0d*2-1,
	0x08*2-1, 0x0e*2-1, 0x10*2-1, 0x0f*2-1
};

inline int GET_FREQ(unsigned char* R) {
	return (((R[3]&0x07)<<8)|R[2]);
}
inline unsigned char GET_WAVETYPE(unsigned char* R) {
	return ((R[0]>>6)&0x03);
}
inline unsigned char GET_SNDLENGTH(unsigned char* R) {
	return (SoundLengthTbl[(R[3]>>3)&0x1f]);
}
inline int GET_SQxLENGTH(unsigned char* R) {
	return (R[0]&0x20)? -1: GET_SNDLENGTH(R);
}
inline int GET_VOLUME(unsigned char* R) {
	return (R[0]&0x10)? ((R[0]&0x0f)*VOL_FADE): VOL_MAX;
}
inline int GET_FADEVALUE(unsigned char* R) {
	return (R[0]&0x10)? -1: ((R[0]&0x0f)+1);
}
inline int GET_SWEEPVALUE(unsigned char* R) {
	return (R[1]&0x80)? (((R[1]&0x70)>>4)+1): -1;
}
inline unsigned char GET_SWEEPSPEED(unsigned char* R) {
	return (R[1]&0x07);
}
inline unsigned char GET_SWEEPDIRECTION(unsigned char* R) {
	return ((R[1]>>3) & 0x01);
}
...
void NesAPU::doSqSweep(unsigned char* regs, int num)
{
			...
			if ((mToggle==3) && (i<=1)) {
				if ((mSweepCounter[i]>0) && (--mSweepCounter[i]==0)) {
					...
					if (mSweepDirection[i]) {
						if (mSweepSpeed[i]) {
							if (i == 0) freq += ~(freq >> mSweepSpeed[0]);
							else        freq -=  (freq >> mSweepSpeed[1]);
						}
					}
					else {
						if (mSweepSpeed[i]) freq += (freq >> mSweepSpeed[i]);
					}
